...
double BroodForce::operator()(const std::vector<Point>& G)
{
	...
	for (size_t i1 = 0; i1 < G.size(); ++i1)
	{
		...
		for (size_t i2 = 0; i2 < G.size(); ++i2)
		{
			...
			visited[i2] = true;
			tmp += dst(G[i1],G[i2]);
			if (G.size() == 2 && (tmp - res < eps)) res = tmp;
			...
			for (size_t i3 = 0; i3 < G.size(); ++i3)
			{
				...
				visited[i3] = true;
				tmp += dst(G[i2], G[i3]);
				if (G.size() == 3 && (tmp - res < eps)) res = tmp;
				...
				for (size_t i4 = 0; i4 < G.size(); ++i4)
				{
					...
					visited[i4] = true;
					tmp += dst(G[i3], G[i4]);
					if (G.size() == 4 && (tmp - res < eps)) res = tmp;
					...
					for (size_t i5 = 0; i5 < G.size(); ++i5)
					{
						...
						visited[i5] = true;
						tmp += dst(G[i4], G[i5]);
						if (G.size() == 5 && (tmp - res < eps)) res = tmp;
						...
						for (size_t i6 = 0; i6 < G.size(); ++i6)
						{
							...
							visited[i6] = true;
							tmp += dst(G[i5], G[i6]);
							if (G.size() == 6 && (tmp - res < eps)) res = tmp;
							...
							for (size_t i7 = 0; i7 < G.size(); ++i7)
							{
								...
								visited[i7] = true;
								tmp += dst(G[i6], G[i7]);
								if (G.size() == 7 && (tmp - res < eps)) res = tmp;
								if (G.size() == 7) continue;
								for (size_t i8 = 0; i8 < G.size(); ++i8)
								{
									if (visited[i8]) continue;
									visited[i8] = true;
									tmp += dst(G[i7], G[i8]);
									if (G.size() == 8 && (tmp - res < eps)) res = tmp;
									...
									for (size_t i9 = 0; i9 < G.size(); ++i9)
									{
										...
										visited[i9] = true;
										tmp += dst(G[i8], G[i9]);
										if (G.size() == 9 && (tmp - res < eps)) res = tmp;
										if (G.size() == 9) continue;
										for (size_t i10 = 0; i10 < G.size(); ++i10)
										{
											if (visited[i10]) continue;
											tmp += dst(G[i9], G[i10]);
											if (G.size() == 10 && (tmp - res < eps)) res = tmp;
										}
										visited[i9] = false;
									}
									visited[i8] = false;
